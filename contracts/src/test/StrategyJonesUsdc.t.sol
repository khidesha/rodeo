...
contract StrategyJonesUsdcTest is DSTest {
    ...
    function setUp() public {
        ...
        s = new StrategyJonesUsdc(address(sh), address(proxy), address(adapter));
    ...
    function testMint() public {
        assertEq(s.mint(address(usdc), 100e6, ""), 100e18);
        usdc.transfer(address(s), 15e6);
        assertEq(s.mint(address(usdc), 50e6, "") / 1e16, 4347);
    }

    function testRate() public {
        s.mint(address(usdc), 100e6, "");
        s.earn();
        assertEq(s.rate(s.totalShares()), 101082000000000000000);
    }

    function testBurn() public {
        s.mint(address(usdc), 100e6, "");
        s.earn();
        assertEq(s.burn(address(usdc), s.totalShares() * 5 / 100, ""), 5054100);
    }

...
contract InvestorTest is Test, ERC721TokenReceiver {
    ...
    function testEarn() public {
        ...
        assertEq(usdcPool.balanceOf(address(0)) - before, 1999);
    }
    ...
    function testKill() public {
        ...
        assertEq(usdc.balanceOf(address(usdcPool)), 900047500);
        ...
        strategy1.file("rate", 21e17);
        uint256 before = usdcPool.balanceOf(address(0));
        investor.kill(id, "");
        assertEq(usdc.balanceOf(vm.addr(2)), 26250);
        assertEq(usdc.balanceOf(address(usdcPool)), 900073749);
        assertEq(usdcPool.balanceOf(address(0)) - before, 26249);
        assertEq(usdcPool.balanceOf(address(0)), 73750);
    }

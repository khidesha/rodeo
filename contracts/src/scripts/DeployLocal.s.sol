...
contract DeployLocal is DSTest {
    function run() external {
        ...
        Pool usdcPool =
            new Pool(address(usdc), address(poolRateModel), address(usdcOracle), 1e6, 90e16, 95e16, 1000000e6);
        Pool wethPool =
            new Pool(address(weth), address(poolRateModel), address(wethOracle), 1e16, 80e16, 90e16, 1000e18);
        ...
        st.file("exec", address(investorActor));
